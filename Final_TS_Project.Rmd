...
$$(1 - 0.1845B - 0.5098B^2 - 0.3896B^3 - 0.0227B^4 + 0.3018B^5)(X_t + 4.470451e-16) = (1 + 0.1845B - 0.4290B^2 - 0.7555B^3)a_T$$

# ARIMA (5,1,3)

## AIC Model Determineation
- We don't see ARMA(5,3) in the top options.
 - After playing around with other options we found ARMA(5,3) to work the best
```{r, echo=FALSE}
...
```{r, echo=FALSE}
diff_est = est.arma.wge(season, p=5, q=3)
```
...
## ARUMA(5,1,1) w/ s = 52 Factor Table Check | Model Estimates
```{r, echo=FALSE}
season_est = est.arma.wge(season, p=5, q=1)

#monthly_est = est.arma.wge(season, p=6, q=1)
```

### RESIDUAL CHECKS ARUMA (5,1,1) w/ s=52
...
- Fail to reject the Null Hypothesis of White Noise as the p-value is greater then .05 for both tests
 - This is another pass for this model
```{r, include=FALSE}
season_l1 = ljung.wge(season_est$res, p=5, q=1)
```
```{r, echo=FALSE}
cat("p-value",season_l1$pval)
```
...
```{r, include=FALSE}
season_l2 = ljung.wge(season_est$res, p=5, q=1, K=48)
```
```{r, echo=FALSE}
cat("p-value",season_l2$pval)
```

## Forecast ARUMA (5,1,1) w/ s=52 Model
```{r, echo=FALSE, results='hide'}
...
#monthly_ase = mean((scaled_test$weekly_sales - monthly_for$f)^2)


#monthly_ase #  3.827409
```
...
## FINAL ARUMA (5,1,1) w/ s=52 MODEL
$$(1 - 0.1845B - 0.5098B^2 - 0.3896B^3 - 0.0227B^4 + 0.3018B^5)(X_t + 4.470451e-16) = (1 + 0.1845B - 0.4290B^2 - 0.7555B^3)a_T$$
...
spn_for = fore.sigplusnoise.wge(scaled$weekly_sales, linear = T, max.p = 7, n.ahead = 8, lastn = T, limits = T) # ASE: 9.038707e+14
```

### ASE
- ARMA(5,3) is my best model still when reviewing ASE
```{r, echo=FALSE, results='hide'}
...
# Calculate WMAE
wmae <- sum(weights * abs(scaled_test$weekly_sales - spn_for$f)) / sum(weights)

# Print the result
wmae
```

## FINAL SIGNAL PLUS NOISE MODEL
$$(1 - 0.1845B - 0.5098B^2 - 0.3896B^3 - 0.0227B^4 + 0.3018B^5)(X_t + 4.470451e-16) = (1 + 0.1845B - 0.4290B^2 - 0.7555B^3)a_T$$

# ARIMA MULTIVARIANT MODEL

## Cross-Correlation Plots
- In this case we are looking for the longest lag to determine how to lag Multivariate Models 
- Since we know Holiday is important and our Lags are across the board, we deceicded to keep it simple and lag everything by 1
```{r, echo=FALSE}
...
- will use this as the base line for the multivariant model
```{r, echo=FALSE, results='hide'}
ksfit = lm(scaled$weekly_sales[2:114] ~ scaled$temp1[2:114] + scaled$fuel_price1[2:114] +  scaled$Unemployment1[2:114] + scaled$CPI1[2:114] + seq(2,114,1)) 
...
## Forecast ARIMA (0,0,0) w/ s=52
- Even though AIC and BIC model this as either ARMA or AR
- We had original seen in the begin that the model could have noise in it
 - We are taking this under consideration and ignoring our AIC and BIC model values for the "ORDER"
```{r, echo=FALSE, results='hide'}
time = seq(1,114,1)
...
- Fail to reject the Null Hypothesis of White Noise as the p-value is greater then .05 for both tests
 - This is another pass for this model
```{r, include=FALSE}
arimaU_l1 = ljung.wge(fit_lag$res, p=0, q=0)
```
```{r, echo=FALSE}
cat("p-value",season_l1$pval)
```
...
```{r, include=FALSE}
arimau_l2 = ljung.wge(fit_lag$res, p=0, q=0, K=48)
```
```{r, echo=FALSE}
cat("p-value",season_l2$pval)
```
## ARIMA(0,0,0) w/ s=52 Predictions
```{r}
last8 = data.frame(
...
#get predictions
preds = predict(fit_lag,newxreg = last8)
```


### ASE
```{r}
ASE = mean((scaled$weekly_sales[107:114] - preds$pred)^2)
ASE 
```
...
## FINAL ARIMA(0,0,0) w/ s=52 MODEL
$$(1 - 0.1845B - 0.5098B^2 - 0.3896B^3 - 0.0227B^4 + 0.3018B^5)(X_t + 4.470451e-16) = (1 + 0.1845B - 0.4290B^2 - 0.7555B^3)a_T$$
...
## VAR Model Factor Table Check | Model Estimates
...
## MLP Model Factor Table Check | Model Estimates
...
# Model Decission
- Need to update this code for all ASE and then do again for all WMAE
```{r, echo=FALSE, results='hide'}
# Creating a data frame to hold the model results
model_results <- data.frame(
  Model = c("ARMA", "ARIMA", "MLP"),
  ASE = c(arma_ase, month_ase, mlp_ASE)  # Ensure these variables exist and are numeric
)

# Sort the data frame by ASE from smallest to largest
model_results <- model_results[order(model_results$ASE), ]

# Display the sorted data frame
#print(model_results)
```

- This will give a nice little bar chart of metric scores for us to talk to
```{r, echo=FALSE, results='hide'}
# Creating a bar chart to compare ASE scores for the models
ggplot(model_results, aes(x = Model, y = ASE, fill = Model)) +
